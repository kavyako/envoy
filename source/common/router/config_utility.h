...
public:
  ...
  /**
   * @return the HeaderMatchType parsed from proto.
   */
  static HeaderMatchType
  parseHeaderMatchSpecifier(const envoy::api::v2::route::HeaderMatcher& config);

  struct HeaderData {
    // HeaderMatcher will consist of one of the below two fields :
    ...
    // 2) any one of exact_match, regex_match or range_match values.
    ...
    HeaderData(const envoy::api::v2::route::HeaderMatcher& config) : name_(config.name()) {
      ...
      case envoy::api::v2::route::HeaderMatcher::kRangeMatch:
        ...
        range_ = Int64Range(config.range_match().start(), config.range_match().end());
    ...
    struct Int64Range {
      Int64Range() {}
      Int64Range(int64_t start, int64_t end) : start_(start), end_(end) {}
